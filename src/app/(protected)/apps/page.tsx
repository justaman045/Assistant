            ...
            {apps.map((app) => {
              ...
              if (!available) {
                return (
                  <div
                    key={app.id}
                    className="flex flex-col gap-4 rounded-xl border border-dashed border-gray-200 bg-gray-50/50 p-5 opacity-70 dark:border-gray-700 dark:bg-gray-900/30"
                  >
              ...
              return (
                <Link
                  key={app.id}
                  href={`/apps/${app.id}`}
                  className="group flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-all duration-150 hover:border-indigo-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-900 dark:hover:border-indigo-700"
                >
                  <div className="flex items-start justify-between">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${app.accentBg}`}>
                      <Icon className={`h-5 w-5 ${app.accentText}`} size={20} />
